<template>
  <div class="flex items-center space-x-4">
    <div class="lg:max-w-[336px] w-full flex items-center relative px-5 py-3 border border-[#0c66ee] rounded-xl">
      ...
      <input :type="type"
        class="w-full text-lg font-medium text-right border-none ring-0 focus:outline-none focus:ring-0" :name="name"
        :value="defaultValue" />
    </div>
        ...
        <ul v-if="openDropdown"
          ...
          <li v-for="exchange in exchanges" id="listbox-option-0" :key="exchange.name"
            ...
            <div class="flex items-center">
              <img :src="getImageUrl(exchange.img)" alt=""
...
export default {
  ...
  setup(props) {
    ...
    const exchangeSelectedf = ref(getImageUrl(props.exchangeSelected.img));
