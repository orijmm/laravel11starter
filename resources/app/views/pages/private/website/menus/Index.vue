    ...
    <Page :title="page.title" :breadcrumbs="page.breadcrumbs" :actions="page.actions" @action="onPageAction">
        ...
        <template #default>
            <Table :id="page.id" v-if="table" :headers="table.headers" :sorting="table.sorting" :actions="table.actions"
                ...
                <template v-slot:content-menus="props">
                    <div>
                        <span  class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800" v-for="(entry, index) in props.item.menus" :key="index">
                            {{ entry.name }}<span v-if="index < props.item.menus.length - 1"> </span>
                        </span>
                    </div>
                </template>
            </Table>
