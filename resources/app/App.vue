        ...
        <aside class="relative bg-theme-600 h-screen w-64 hidden sm:block shadow-xl">
            <div class="p-6 border-b border-theme-600">
                ...
                <template v-if="state.headerLeftLink">
                    <a v-if="state.headerLeftLink.href" :href="state.headerLeftLink.href"
                        class="w-full bg-white cta-btn font-semibold py-2 mt-5 rounded-br-lg rounded-bl-lg rounded-tr-lg shadow-lg hover:shadow-xl hover:bg-gray-300 flex items-center justify-center">
                        <Icon :name="state.headerLeftLink.icon" class="mr-3" />
                        {{ state.headerLeftLink.name }}
                    </a>
                    <router-link v-else :to="state.headerLeftLink.to"
                        class="w-full bg-white cta-btn font-semibold py-2 mt-5 rounded-br-lg rounded-bl-lg rounded-tr-lg shadow-lg hover:shadow-xl hover:bg-gray-300 flex items-center justify-center">
                        <Icon :name="state.headerLeftLink.icon" class="mr-3" />
                        {{ state.headerLeftLink.name }}
                    </router-link>
            ...
            <template v-if="state.footerLeftLink">
                <a v-if="state.footerLeftLink.href" :href="state.footerLeftLink.href"
        ...
        <div class="relative w-full flex flex-col h-screen overflow-y-hidden">
            ...
            <header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
                ...
                <div class="relative w-1/2 flex justify-end">
                    <a class="flex cursor-pointer focus:outline-none align-middle"
                        ...
                        <button
                            ...
                            <img :alt="authStore.user.full_name" v-if="authStore.user.avatar_thumb_url"
                                :src="authStore.user.avatar_thumb_url">
                            <AvatarIcon v-else />
                        </button>
                    </a>
                    <button v-if="state.isAccountDropdownOpen" @click="state.isAccountDropdownOpen = false"
                        class="h-full w-full fixed inset-0 cursor-pointer"></button>
                    <div v-if="state.isAccountDropdownOpen"
            ...
            <header class="w-full bg-theme-600 py-5 px-6 sm:hidden">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="state.isMobileMenuOpen = !state.isMobileMenuOpen"
                        class="text-white text-3xl focus:outline-none">
                        <i v-if="!state.isMobileMenuOpen" class="fa fa-bars"></i>
                        <i v-else class="fa fa-times"></i>
                    </button>
                </div>
                <nav :class="state.isMobileMenuOpen ? 'flex' : 'hidden'"
        ...
        const state = reactive({
            mainMenu: [
                ...
                },
                {
                    ...
                    children: [
                        {
                            name: trans('global.pages.general_info'),
                            icon: 'tachometer',
                            showDesktop: true,
                            showMobile: true,
                            requiresAbility: getAbilitiesForRoute(['settingad.edit']),
